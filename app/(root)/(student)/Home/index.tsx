...
const Home = () => {
  ...
  const [filteredCourses, setFilteredCourses] = useState<GetCourses[]>(courses);
  const coursePreview = [...filteredCourses].reverse().slice(0, 4);
    ...
    <SafeAreaView className="w-full">
      <View className="w-full sticky bg-white py-[10px] px-5">
        ...
        <View className="mt-5 border-[0.7px] border-[#E5E5E5] rounded-[15px] px-[10px] py-[4px] bg-white flex flex-row justify-between items-center">
          ...
          >
            <Setting4 size="18" color="#FFffff" />
          </TouchableOpacity>
        </View>
      </View>
      <ScrollView className="w-full h-[640px] px-5 py-3">
        <DashboardHeading
          title="Categories"
          link="/(student)/AllCategories"
          linkText="See all"
        />
          ...
          <View className="mb-4">
            <View className="flex flex-row flex-wrap justify-between">
              {coursePreview.length > 0 ? (
                ...
                <View className="flex-1 justify-center items-center py-8 w-full">
      ...
      <FilterBottomSheet filterRef={bottomSheetModalRef} />
    </SafeAreaView>
  );
};
