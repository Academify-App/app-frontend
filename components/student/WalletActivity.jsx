  ...
  return (
    <View className="w-full pl-3 bg-white pr-3">
      <View className="flex-row justify-between mt-4">
        ...
        <Link href='/(student)/(tabs)/(wallet)/history'>
      ...
      {data.map((item,index)=>(
          ...
          <View>
            <View className="flex-row">
              ...
              <View className="w-[40px] h-[40px] rounded-full bg-red-200 flex justify-center items-center">
                <MaterialIcons name="arrow-outward" size={24} color="red" />
              </View>
              )
              :
              (
        ...
        <View>
        { item.expense === "negative" ? (
          <Text className={`${getFontSize()} text-red-500 font-robotomedium`}>+N{item.amount}</Text>
        )
        :
        (
