...
import { setCategories, setLevels, setPriceRange, setRatings, resetFilters, setAllFilters } from '@/store/slices/filterSlice';
...
const FilterBottomSheet: React.FC<FilterBottomSheetProps> = ({ onApplyFilters, filterRef }) => {
                                    ...
                                    {categoryOptions.map((item, index) => (
                                        ...
                                        >
                                            <Checkbox
                                                ...
                                                color={filters.categories.includes(item.value) ? '#250F53' : undefined}
                                                className="mr-2"
                                            />
                                            <Text className={`${filters.categories.includes(item.value) ? 'text-[#250F53]' : 'text-[#6B6B6B]'} text-sm`}>
                                                {item.label}
                                            </Text>
                                        </TouchableOpacity>
                                    ))}
                                    ...
                                    {levelOptions.map((item, index) => (
                                        ...
                                        >
                                            <Checkbox
                                                ...
                                                color={filters.levels.includes(item.value) ? '#250F53' : undefined}
                                                className="mr-2"
                                            />
                                            <Text className={`${filters.levels.includes(item.value) ? 'text-[#250F53]' : 'text-[#6B6B6B]'} text-sm`}>
                                                {item.label}
                                            </Text>
                                        </TouchableOpacity>
                                    ))}
                                    ...
                                    {ratingOptions.map((item, index) => (
                                        ...
                                        >
                                            <Checkbox
                                                ...
                                                color={filters.ratings.includes(item.value) ? '#250F53' : undefined}
                                                className="mr-2"
                                            />
                                            <Text className={`${filters.ratings.includes(item.value) ? 'text-[#250F53]' : 'text-[#6B6B6B]'} text-sm`}>
                                                {item.label}
                                            </Text>
                                        </TouchableOpacity>
                                    ))}
